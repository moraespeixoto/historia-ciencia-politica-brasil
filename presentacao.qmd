---
title: "Sessenta Anos Depois: Teses, Programas e Agendas da Ciência Política no Brasil"
...
::: {.notes}
Este slide corrige a leitura corrente. Com os códigos de área de avaliação da Sucupira conferidos contra o campo `NM_AREA_AVALIACAO` (28 Economia, 34 Sociologia, 35 Antropologia/Arqueologia, 36 Geografia, 39 CP/RI, 40 História), a Área 39 aparece à frente de `r lista_areas(prog24[5:6])` e atrás de `r lista_areas(prog24[1:3])`.
...
::: {.fragment .notes}
Modelo STM com $K = `r v_stm$K`$ sobre `r fmt_int(v_stm$modelo_docs)` resumos **em português**. O acervo bruto tem `r fmt_int(v_stm$corpus_bruto_n)` resumos; `r fmt_int(v_stm$n_descartados_idioma)` não estão em português e ficaram de fora da modelagem lexical (mas seguem nas séries de títulos e artigos). Um corpus multilíngue produziria dois tópicos que seriam vocabulário de idioma, não linhas de pesquisa; restringir ao português evita esse artefato.
